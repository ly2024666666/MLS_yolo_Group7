...
# GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, Focal=False
def bbox_iou(box1, box2, xywh=True,lossname="",Focal=False, alpha=1, gamma=0.5, eps=1e-7):
    ...
    if lossname in ["CIoU","DIoU","GIoU","EIoU","SIoU", "MPDIoU"]:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if lossname in ["CIoU","DIoU","EIoU","SIoU"]:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            c2 = (cw ** 2 + ch ** 2) ** alpha + eps  # convex diagonal squared
            rho2 = (((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 + (b2_y1 + b2_y2 - b1_y1 - b1_y2) ** 2) / 4) ** alpha  # center dist ** 2
            if lossname == "CIoU":  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
            ...
            elif lossname == "MPDIoU":
                perim1 = 2 * (w1 + h1)
                perim2 = 2 * (w2 + h2)
                perim_enclose = 2 * (cw + ch)
                perim_term = alpha * ((perim_enclose - torch.min(perim1, perim2)) / perim_enclose.clamp(min=eps))
                if Focal:
                    return iou - rho2 / c2 - perim_term, torch.pow(inter / union, gamma)
                else:
                    return iou - rho2 / c2 - perim_term        
            if Focal:
