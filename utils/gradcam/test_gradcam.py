...
def test_gradcam():
    ...
    try:
        # 1. 创建模型
        print("1. 加载YOLOv5模型...")
        model = YOLOV5TorchObjectDetector(model_weight=model_path, device=device, img_size=img_size, names=names)
        print("   模型加载成功!")
        ...
        try:
            YOLOV5GradCAM(model=model, layer_name=target_layer, img_size=img_size)
            print("   GradCAM初始化成功!")
        ...
        print("5. 测试GradCAM++...")
        try:
            YOLOV5GradCAMPP(model=model, layer_name=target_layer, img_size=img_size)
            print("   GradCAM++初始化成功!")
        except Exception as e:
