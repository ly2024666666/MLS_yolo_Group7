...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    if label:
        ...
        cv2.putText(
            img,
            label,
            (c1[0], c1[1] - 2 if outside else c2[1] - 2),
            0,
            tl / 3,
            [225, 255, 255],
            thickness=tf,
...
def main(img_path):
    ...
    for target_layer in target_layers:
        ...
        for i, mask in enumerate(masks):
            ...
            res_img = plot_one_box(
                bbox, res_img, label=label, color=colors[int(names.index(cls_name))], line_thickness=3
            )
            # 缩放到原图片大小
